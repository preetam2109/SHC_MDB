...
@Component({
  selector: 'app-warehouse-indent',
  templateUrl: './warehouse-indent.component.html',
  styleUrls: ['./warehouse-indent.component.css']
})
export class WarehouseIndentComponent implements OnInit {
...
this.spinner.show()
  ...
  getHCitems(category:number) {
    ...
    this.api.getSHCitems(category,this.nocid).subscribe(
      (res: any) => {
        this.sHCitems = res;
  ...
  saveData() {
    ...
    if (!this.nocid) {
      alert('NOC ID is missing. Please check the input.');
      return;
    }
    ...
    itemsToSave.forEach((itemToSave, index) => {
      ...
      if (matchingItem) {
        const nocItemData: NOCItems = {
          sr: matchingItem.sr, // Pass the sr value
          itemid: itemToSave.itemid,
          nocid: this.nocid,
          requestedqty: parseInt(itemToSave.requestedqty, 10) / parseInt(itemToSave.unitcount, 10),
          whstock: 0,
          cgmsclremarks: '',
          status: 'C',
          approvedqty: 0,
          bookedqty: itemToSave.requestedqty,
          bookedflag: 'B',
          stockinhand: 0,
        };

        // Determine whether to insert or update
        const isInsert = nocItemData.sr === 0;
debugger
return
        this.api.postNOCitems(nocItemData, nocItemData.sr).subscribe(
          () => {
            if (isInsert) {
              saveCount++;
            } else {
            ...
            // If this is the last iteration, show the appropriate message and set saved flag
            if (index === itemsToSave.length - 1) {
              if (saveCount > 0 && updateCount > 0) {
                alert(`${saveCount} items added and ${updateCount} items updated successfully!`);
              } else if (saveCount > 0) {
                alert(`${saveCount} items added successfully!`);
  ...
  freezData() {
    ...
    const allItemsZero = this.sHCitems.every(item => item.indentqty === 0);
    if (allItemsZero) {
      alert('* फ्रीज करने से पहले मांग में सामग्री जोड़े\n* Before Freezing, Please Add/Update items in the indent.');
      return; // Exit the method entirely
    }
    //  Second, check if data has been saved
     if (!this.isDataSaved) {
      alert('Please Add/Update the indent data first before freezing.');
      return;
    }
   
    this.spinner.show();
    this.api.freezeNOCitems(this.nocid).subscribe(
      (res: any) => {
        alert('मांग सफलतापूर्वक फ्रीज कर पैरेंट फैसिलिटी को भेज दिया गया \n Indent successfully sent to parent facility');
        this.spinner.hide();
        this.router.navigate(['/indent-to-warehouse']);

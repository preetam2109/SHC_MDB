...
@Component({
  selector: 'app-facility-wise-issue',
  standalone: true,
  ...
  templateUrl: './facility-wise-issue.component.html',
  styleUrl: './facility-wise-issue.component.css'
})
export class FacilityWiseIssueComponent {

  dataSource!: MatTableDataSource<AAMAdminFacilityWiseIssue>;
  aAMAdminFacilityWiseIssue: AAMAdminFacilityWiseIssue[] = [];
  ...
  constructor(
    ...
    this.dataSource = new MatTableDataSource<AAMAdminFacilityWiseIssue>([]);
...
const formattedEndDate = endDate ? this.datePipe.transform(endDate, 'dd-MMM-yyyy') : '';
  ...
  if (formattedStartDate && formattedEndDate) {
    this.spinner.show();
    this.api.getFacilityWiseIssue(sessionStorage.getItem('districtid'), 0, 0, formattedStartDate, formattedEndDate).subscribe(
      (res: AAMAdminFacilityWiseIssue[]) => {
        if (res.length === 0) {
  ...
  doc.text(`Date: ${dateString} Time: ${timeString}`, 10, 10); // Top-left at position X=10, Y=10
    const columns = [
      ...
      { title: "Issue Type", dataKey: "wardname" },
      { title: "Mfg DT", dataKey: "mfgdate" },
      { title: "Exp DT", dataKey: "expdate" },
      { title: "Issue DT", dataKey: "issuedate" },
      { title: "Issue Qty", dataKey: "issueqty" },
    ];
    
    const rows = this.aAMAdminFacilityWiseIssue.map(row => ({
      sno: row.sno,

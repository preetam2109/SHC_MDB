...
@Component({
  ...
  templateUrl: './kpidist-wise.component.html',
  styleUrl: './kpidist-wise.component.css'
})
export class KPIdistWiseComponent {

  dataSource!: MatTableDataSource<KPIdistWise>;
  kPIdistWise: KPIdistWise[] = [];
  ...
  //       this.kPIdistWise = res.map((item, index) => ({
  //         ...item,
  //         sno: index + 1
  //       }));
  ...
  // }
    ...
    this.spinner.show();
    this.api.AAMHealthPerformance().subscribe(
      (res) => {
        // Add serial numbers to the data
...
// console.log(JSON.stringify(res))
  ...
  exportToPDF() {
    ...
    const header='KPIdistWise Stock Report'
    ...
    const columns = [
      ...
      { title: "No. of AAM Facilities", dataKey: "nooffac" },
      { title: "No. of AAM Opening Stock Entered", dataKey: "noofop" },
      { title: "No. of AAM Indent to Warehouse", dataKey: "nosindentwh" },
      { title: "No. of AAM Received from Warehouse", dataKey: "nosfacilityreceipt" },
    ...
    doc.save('KPIdistWise_Report.pdf');
  }

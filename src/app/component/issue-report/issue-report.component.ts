...
import { ConsumptionReport } from 'src/app/Model/ConsumptionReport';
...
export class IssueReportComponent {
  ...
  dataSource!: MatTableDataSource<ConsumptionReport>;
  consumptionR: ConsumptionReport[] = [];
  ...
  constructor(
    ...
    this.dataSource = new MatTableDataSource<ConsumptionReport>([]);
...
// 
//       this.api.getConsumptionReport('B',this.facid,0,formattedStartDate,formattedEndDate).subscribe(
...
const formattedEndDate = endDate ? this.datePipe.transform(endDate, 'dd-MMM-yyyy') : '';
  ...
  exportToPDF() {
    ...
    const header = 'Consumption Report';
  ...
  doc.text(`Date: ${dateString} Time: ${timeString}`, 10, 10); // Top-left at position X=10, Y=10
    ...
    const rows = this.consumptionR.map(row => ({
      sno: row.sno,
